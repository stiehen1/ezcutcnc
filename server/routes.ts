  ...
  app.post("/api/roi", async (req, res) => {
    try {
      const {
        ...
        ccLifeUnits, compLifeUnits,
        ccToolCost: ccToolCostIn, ccMachineCost: ccMachineCostIn, ccTotalCost: ccTotalCostIn,
        ...
        ccLifeUnits?: number; compLifeUnits?: number;
      ...
      try {
        ...
        await pool.query(
          ...
          DO UPDATE SET
            ...
            distributor_contact = EXCLUDED.distributor_contact,
            distributor_address = EXCLUDED.distributor_address,
            distributor_city = EXCLUDED.distributor_city,
            distributor_state = EXCLUDED.distributor_state,
          ...
          [
            ...
            // $74–$78: distributor "Generated by" block (opt-in)
            distributorContact || null, distributorAddress || null,
      ...
      if (smtpUser && smtpPass && userEmail) {
        ...
        // Prefer the client-computed cost breakdown (single source of truth — it
        // includes the MRR machine-time term). Fall back to a local computation
        // only for older clients / direct API callers that don't send it.
        const ccToolCost = ccToolCostIn ?? ((ccToolPrice ?? 0) / (ccPartsPer ?? 1));
        const ccMachineCost = ccMachineCostIn ?? (((ccTimeInCut ?? 0) / 60) * (shopRate ?? 0));
        const ccTotalCost = ccTotalCostIn ?? (ccToolCost + ccMachineCost);
        const compToolCost = compToolCostIn ?? ((compPrice ?? 0) / (compPartsPer ?? 1));
        const compMachineCost = compMachineCostIn ?? (((compTimeInCut ?? 0) / 60) * (shopRate ?? 0));
        const compTotalCost = compTotalCostIn ?? (compToolCost + compMachineCost);

        // "Generated by" distributor block — only when a distributor company was
        // supplied via the opt-in toggle. HTML-escaped to stay safe in the email.
        const esc = (s: string) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
        const distLines: string[] = [];
        if (distributorContact && distributorContact.trim()) distLines.push(esc(distributorContact.trim()));
        if (distributorAddress && distributorAddress.trim()) distLines.push(esc(distributorAddress.trim()));
        const cityStateZip = [distributorCity, distributorState].filter(v => v && v.trim()).map(v => esc(v!.trim())).join(", ")
          + (distributorZip && distributorZip.trim() ? ` ${esc(distributorZip.trim())}` : "");
        if (cityStateZip.trim()) distLines.push(cityStateZip.trim());
        const distributorBlockHtml = (distributorName && distributorName.trim())
          ? `<td style="vertical-align:top;text-align:right;font-family:Arial,sans-serif;">
              <div style="font-size:11px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.5px;margin:0 0 6px;">Generated by</div>
              <div style="font-size:13px;color:#111827;font-weight:700;margin:0 0 3px;">${esc(distributorName.trim())}</div>
              ${distLines.map(l => `<div style="font-size:12px;color:#4b5563;margin:0 0 2px;">${l}</div>`).join("")}
            </td>`
          : "";

        // Light / white email theme — renders reliably across Outlook (Word engine),
        // Gmail, and Apple Mail. Table-based layout, no flexbox, no border-radius reliance.
        const rowGray = "background:#f4f4f5;";
        const cellL = "padding:7px 12px;font-size:13px;color:#6b7280;font-family:Arial,sans-serif;";
        const cellR = "padding:7px 12px;font-size:13px;color:#111827;font-family:Arial,sans-serif;";
        const cmpTd = "padding:9px 12px;font-size:13px;font-family:Arial,sans-serif;border-bottom:1px solid #eee;";
        // Per-KPI delta cell: % change of CC vs competitor, with an up/down arrow.
        // Arrow = raw direction (↑ CC higher, ↓ CC lower). Color = good/bad for the
        // customer: higherIsBetter metrics (MRR, parts/tool) are green when up;
        // cost/time metrics are green when down. Renders "—" if either side is missing.
        const deltaCell = (cc?: number | null, comp?: number | null, higherIsBetter = false, extra = "") => {
          const c = Number(cc), p = Number(comp);
          if (!Number.isFinite(c) || !Number.isFinite(p) || p === 0) return `<td style="${cmpTd}${extra}text-align:right;color:#9ca3af;">—</td>`;
          const pct = ((c - p) / Math.abs(p)) * 100;
          if (Math.abs(pct) < 0.05) return `<td style="${cmpTd}${extra}text-align:right;color:#6b7280;">—</td>`;
          const up = c > p;
          const good = higherIsBetter ? up : !up;
          const color = good ? "#15803d" : "#dc2626";
          const arrow = up ? "▲" : "▼";
          return `<td style="${cmpTd}${extra}text-align:right;font-weight:700;color:${color};white-space:nowrap;">${arrow} ${fmtD(Math.abs(pct))}%</td>`;
        };
        // Life-mode–aware labels so the comparison table matches whatever ROI
        // measurement the user chose (parts / cut-time / linear inches).
        const _lm = lifeMode === "cut_time" ? "cut_time" : lifeMode === "linear_in" ? "linear_in" : "parts";
        const lifeLabel = _lm === "cut_time" ? "Cut Time per Tool (min)" : _lm === "linear_in" ? "Linear Inches per Tool" : "Parts per Tool";
        const perUnit = _lm === "cut_time" ? "Min" : _lm === "linear_in" ? "Inch" : "Part";
        const ccLife = ccLifeUnits ?? ccPartsPer;
        const compLife = compLifeUnits ?? compPartsPer;

        const htmlBody = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="font-family:Arial,sans-serif;background:#f0f0f0;color:#111827;margin:0;padding:0;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f0f0f0;">
    <tr><td align="center" style="padding:28px 12px;">
      <table role="presentation" width="620" cellpadding="0" cellspacing="0" style="width:620px;max-width:620px;background:#ffffff;border:1px solid #e5e7eb;">
        <!-- Header -->
        <tr>
          <td style="background:#ffffff;border-bottom:3px solid #ea580c;padding:18px 28px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td style="vertical-align:middle;width:210px;padding-right:18px;">
                  <img src="https://corecutcnc.com/CCLogo-long-whiteback%20TRANSPARENT.png" alt="Core Cutter — Solid Carbide Cutting Tools" width="210" style="width:210px;max-width:210px;height:auto;display:block;border:0;">
                </td>
                <td style="vertical-align:middle;text-align:right;">
                  <div style="font-size:20px;font-weight:700;color:#111827;letter-spacing:-0.3px;">ROI Summary</div>
                  <div style="font-size:13px;color:#6b7280;margin-top:3px;">Your machining cost comparison report</div>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr><td style="padding:24px 28px;">
          ...
          <!-- Job spec table — rows built dynamically so zebra striping stays correct
               regardless of which optional rows (part, feed, EDPs) are present. -->
          ${(() => {
            const rows: [string, string][] = [];
            if (endUserCompany && endUserCompany.trim()) rows.push(["Company", esc(endUserCompany.trim())]);
            if (partName && partName.trim()) rows.push(["Part Name", esc(partName.trim())]);
            if (partNumber && partNumber.trim()) rows.push(["Part Number", esc(partNumber.trim())]);
            rows.push(["Material", material || "—"]);
            rows.push(["Operation", operation || "—"]);
            rows.push(["Tool Diameter", `${fmtFour(toolDia)}"`]);
            if (feedIpm) rows.push(["Feed Rate", `${fmtD(feedIpm)} IPM`]);
            if (ccEdp) rows.push(["CC EDP", String(ccEdp)]);
            if (compEdp) rows.push(["Competitor EDP", String(compEdp)]);
            rows.push(["Shop Rate", `$${fmtD(shopRate)}/hr`]);
            rows.push([_lm === "cut_time" ? "Annual Cutting Hours" : _lm === "linear_in" ? "Annual Linear Inches" : "Annual Volume", `${annualVolume ?? "—"}${_lm === "cut_time" ? " hrs" : _lm === "linear_in" ? " in" : " parts"}`]);
            const body = rows.map(([k, v], i) =>
              `<tr${i % 2 ? ` style="${rowGray}"` : ""}><td style="${cellL}${i === 0 ? "width:42%;" : ""}">${k}</td><td style="${cellR}">${v}</td></tr>`
            ).join("");
            return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:20px;border:1px solid #eee;">${body}</table>`;
          })()}

          <!-- Comparison table — rows built dynamically so labels track the ROI
               life mode, empty rows drop out, and each row carries a Δ column. -->
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:22px;border:1px solid #eee;">
            <tr>
              <th style="padding:10px 12px;text-align:left;background:#f9fafb;color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.4px;font-family:Arial,sans-serif;border-bottom:2px solid #e5e7eb;"></th>
              <th bgcolor="#ea580c" style="padding:10px 12px;text-align:right;background:#ea580c;color:#ffffff;font-size:12px;font-weight:700;font-family:Arial,sans-serif;">Core Cutter</th>
              <th bgcolor="#3f3f46" style="padding:10px 12px;text-align:right;background:#3f3f46;color:#ffffff;font-size:12px;font-weight:700;font-family:Arial,sans-serif;">Competitor</th>
              <th style="padding:10px 12px;text-align:right;background:#f9fafb;color:#6b7280;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.4px;font-family:Arial,sans-serif;border-bottom:2px solid #e5e7eb;">Δ</th>
            </tr>
            ${(() => {
              const has = (v?: number | null) => Number.isFinite(Number(v)) && Number(v) !== 0;
              // [label, ccDisplay, compDisplay, ccNum, compNum, higherIsBetter]
              const rows: [string, string, string, number | undefined, number | undefined, boolean][] = [];
              if (has(ccMrr) || has(compMrr))
                rows.push(["Material Removal Rate", has(ccMrr) ? `${fmtD(ccMrr)} in³/min` : "—", has(compMrr) ? `${fmtD(compMrr)} in³/min` : "—", ccMrr, compMrr, true]);
              rows.push(["Tool Price", `$${fmtD(ccToolPrice)}`, `$${fmtD(compPrice)}`, ccToolPrice, compPrice, false]);
              rows.push([lifeLabel, `${ccLife ?? "—"}`, `${compLife ?? "—"}`, ccLife, compLife, true]);
              rows.push([`Tool Cost / ${perUnit}`, `$${fmtD(ccToolCost)}`, `$${fmtD(compToolCost)}`, ccToolCost, compToolCost, false]);
              if (has(ccMachineCost) || has(compMachineCost))
                rows.push([`Machine Cost / ${perUnit}`, `$${fmtD(ccMachineCost)}`, `$${fmtD(compMachineCost)}`, ccMachineCost, compMachineCost, false]);
              const body = rows.map(([label, ccV, compV, ccN, compN, hib], i) => {
                const stripe = i % 2 ? rowGray : "";
                return `<tr style="${stripe}">
                  <td style="${cmpTd}color:#4b5563;">${label}</td>
                  <td style="${cmpTd}color:#111827;text-align:right;font-weight:600;">${ccV}</td>
                  <td style="${cmpTd}color:#4b5563;text-align:right;">${compV}</td>
                  ${deltaCell(ccN, compN, hib)}
                </tr>`;
              }).join("");
              const totalTd = "padding:10px 12px;font-size:13px;font-family:Arial,sans-serif;font-weight:700;border-top:2px solid #e5e7eb;";
              const totalRow = `<tr bgcolor="#fff7ed" style="background:#fff7ed;">
                <td style="${totalTd}color:#111827;">Total Cost / ${perUnit}</td>
                <td style="${totalTd}color:#ea580c;text-align:right;">$${fmtD(ccTotalCost)}</td>
                <td style="${totalTd}color:#4b5563;text-align:right;">$${fmtD(compTotalCost)}</td>
                ${deltaCell(ccTotalCost, compTotalCost, false, "border-top:2px solid #e5e7eb;")}
              </tr>`;
          ...
          <!-- Savings highlight -->
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="#f0fdf4" style="background:#f0fdf4;border:2px solid #16a34a;margin-bottom:24px;">
            <tr>
              <td width="25%" style="padding:18px 12px;text-align:center;font-family:Arial,sans-serif;">
                <div style="font-size:26px;font-weight:700;color:#15803d;">$${fmtD(savingsPerPart)}</div>
                <div style="font-size:12px;color:#16a34a;">Savings per part</div>
              </td>
              <td width="25%" style="padding:18px 12px;text-align:center;font-family:Arial,sans-serif;">
                <div style="font-size:22px;font-weight:700;color:#15803d;">$${fmtD(monthlySavings)}</div>
                <div style="font-size:12px;color:#16a34a;">Monthly savings</div>
              </td>
              <td width="25%" style="padding:18px 12px;text-align:center;font-family:Arial,sans-serif;">
                <div style="font-size:22px;font-weight:700;color:#15803d;">$${fmtD(annualSavings)}</div>
                <div style="font-size:12px;color:#16a34a;">Annual savings</div>
              </td>
              <td width="25%" style="padding:18px 12px;text-align:center;font-family:Arial,sans-serif;">
                <div style="font-size:22px;font-weight:700;color:#15803d;">${fmtD(savingsPct)}%</div>
                <div style="font-size:12px;color:#16a34a;">Cost reduction</div>
              </td>
            </tr>
          </table>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #e5e7eb;">
            <tr>
              <td style="vertical-align:top;padding-top:16px;font-family:Arial,sans-serif;">
                <div style="font-size:13px;color:#111827;font-weight:700;margin:0 0 6px;">Core Cutter USA</div>
                <div style="font-size:12px;color:#4b5563;margin:0 0 3px;">Phone: <a href="tel:+12075887519" style="color:#ea580c;text-decoration:none;">207-588-7519</a></div>
                <div style="font-size:12px;color:#4b5563;margin:0 0 3px;">Email: <a href="mailto:sales@corecutterusa.com" style="color:#ea580c;text-decoration:none;">sales@corecutterusa.com</a></div>
                <div style="font-size:12px;color:#4b5563;margin:0;">Web: <a href="https://corecutcnc.com" style="color:#ea580c;text-decoration:none;">corecutcnc.com</a></div>
              </td>
              ${distributorBlockHtml ? distributorBlockHtml.replace("vertical-align:top;", "vertical-align:top;padding-top:16px;") : ""}
            </tr>
          </table>
          <div style="font-size:11px;color:#9ca3af;margin:14px 0 0;font-family:Arial,sans-serif;">Generated by CoreCutCNC — figures are starting estimates for comparison.</div>
...
1b. A DIAMETER MUST CARRY A Ø (or ⌀) SYMBOL. A callout is a diameter ONLY if it is prefixed with Ø/⌀. A bare toleranced number with NO Ø symbol is a LENGTH — a step-end/step-length dimension, an LOC, an OAL, a margin/land width, or a bolt-circle spacing — and must NEVER be placed in tool_dia, shank_dia, or drill_step_diameters. This matters most near the cutting end, where a bold linear dimension (e.g. ".3149±.0008" with no Ø, dimensioning the axial length of a step feature) sits right next to real diameter callouts and is easily mistaken for the largest diameter. Before writing any value into a diameter field, confirm the source callout has a Ø prefix. If it does not, it is a length. (Real diameters on Core Cutter prints always show Ø, e.g. "Ø.1634+.0004/-.0002", "Ø.2362-.0001/-.0004".)
...
2b. For STEP DRILLS specifically — this is the most important rule for step drills:
   ...
   - Example C (TOP-CHAMFER plunge step drill — Core Cutter makes these frequently): a two-diameter drill where the tip cuts a pilot hole and the shank OD cuts a chamfer/counterbore at the top of the hole. Only TWO Ø callouts exist: the small tip Ø and the larger shank Ø, both bold. There are NO intermediate steps. The shank Ø IS the largest cutting diameter and governs SFM. Watch for a phantom "largest diameter" that is actually a LENGTH callout with no Ø symbol (see rule 1b) — do NOT use it. Example CC-13410 — Ø.1634 tip (bold, plunging pilot), Ø.2362 shank (bold, cuts the top chamfer), .3149 is a LENGTH (no Ø — IGNORE for diameters), .340 LOC → tool_dia=0.1634, drill_step_diameters=[0.2362], shank_dia=0.2362, drill_flute_length=0.340. The step length (tip to where Ø.2362 begins cutting) goes in drill_step_lengths.
