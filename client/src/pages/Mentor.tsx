...
const MILLING_MODE_TIPS: Record<string, Array<{ title: string; body: string }>> = {
  hem: [
    ...
    { title: "Low engagement requires higher feed — or you're rubbing.", body: "At ~10% WOC, increase IPT 1.5–2.0×. At ~5% WOC, increase IPT 2.0–2.5×. The engine calculates chip thinning and adjusts FPT automatically. If you don't compensate, you rub instead of cut — heat skyrockets and tool life tanks with no obvious cause. In HRSA (Inconel 718, titanium), 3–5% WOC with very high chip-thinning-adjusted feed is proven shop practice — the low radial load keeps heat in the chip and protects the cutting edge." },
    ...
    { title: "Variable pitch and variable helix kill chatter.", body: "These geometry features break up the harmonic frequency that causes chatter. The engine accounts for them in the stability calculation — make sure variable_pitch and variable_helix are set correctly. Gain: up to 1.50× and 1.25× chatter limit respectively, or 1.75× when both are active. Core Cutter lineup: aluminum tools are standard geometry (neither); steel/stainless tools are all variable pitch; QTR3 series is both variable pitch and variable helix — the most chatter-resistant geometry in the line." },
    { title: "Entry strategy — in order of preference.", body: "① Pre-drilled hole (best): drill or bore the entry before HEM passes begin. The tool drops in cleanly, no edge contact at entry, zero chip load spike. Best for deep pockets, tight tolerances, and hard materials. ② Sweep-in / roll-in: tangential arc lead-in from outside the stock — tool engagement builds from zero, very low shock, recommended by the app when a pre-hole isn't used. ③ Helical interpolation: ramp the tool in a continuous helix (circular XY + descending Z) to depth, then open to width with HEM passes. Ramp angle ≤2–3°; set ramp feed to 40–50% of lateral feed; verify center-cutting geometry. ④ Linear ramp: straight-line ramp at 2–5° angle. Simple and widely supported in CAM but builds engagement faster than helical. Use when helix clearance is unavailable. Never straight plunge into full-width material. CAM defaults often spike engagement at inside corners — add corner smoothing and eliminate 90° inside corners from your toolpath." },
  ...
  surfacing: [
    { title: "Pick your Ra target — the app calculates the rest.", body: "The Surface Finish Target (Ra) presets control stepover and axial pass depth automatically. Rough (500 µin) = fast stock removal, visible cusps. Semi (125 µin) = light cusps, good for semi-finish passes. Finish (32 µin) = smooth, typical mold/die. Fine (16 µin) = near-mirror, slow but clean. The app converts Ra to the correct scallop height and stepover for your specific tool diameter — no manual stepover math required." },
...
<script>setTimeout(function(){window.focus();window.print();},800);</script>
  ...
  React.useEffect(() => {
    if (machineQuery.length < 1) { setMachineResults([]); return; }
      ...
      <Card className="rounded-2xl">
        ...
        <CardContent className="space-y-4 pt-0">
          ...
          <div className="mb-4 space-y-2">
            ...
            {savedMachines.length > 0 && (
              <div className="mb-1 rounded-lg border border-zinc-700/60 overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <a href="/toolbox" className="text-[10px] text-zinc-500 hover:text-sky-400 underline underline-offset-2" onClick={e => e.stopPropagation()}>Manage →</a>
                ...
                {savedMachinesOpen && (
                  <div className="px-2 pt-1.5 pb-2 space-y-1">
                    {savedMachines.map(m => {
                      ...
                      const statusColor = status !== "operational" ? (status === "down" ? "text-red-400" : status === "issue" ? "text-amber-400" : "text-blue-400") : "";
                      return (
                        <button
                          key={m.id}
                          type="button"
                          onClick={() => applyMachineToForm(m)}
                          className={`w-full text-left rounded-lg border px-2.5 py-2 transition-colors ${isActive ? "border-orange-500 bg-orange-500/5" : "border-zinc-700 bg-zinc-800/40 hover:border-zinc-500"}`}
                        >
                          <div className="flex items-center gap-1.5">
                            <span>{statusIcon}</span>
                            <span className={`text-xs font-semibold truncate ${isActive ? "text-orange-400" : "text-zinc-200"}`}>
                              {m.nickname}{m.shop_machine_no ? ` #${m.shop_machine_no}` : ""}
                            </span>
                          </div>
                          <div className="flex flex-wrap gap-x-2 mt-0.5 pl-5">
                            {(m.brand || m.model) && <span className="text-[10px] text-zinc-500">{[m.brand, m.model].filter(Boolean).join(" ")}</span>}
                            {m.max_rpm && <span className="text-[10px] text-zinc-600">{m.max_rpm.toLocaleString()} RPM</span>}
                            {m.spindle_hp && <span className="text-[10px] text-zinc-600">{m.spindle_hp} HP</span>}
                            {m.taper && <span className="text-[10px] text-zinc-600">{m.taper}</span>}
                          </div>
                          {status !== "operational" && m.status_note && (
                            <p className={`text-[10px] pl-5 mt-0.5 ${statusColor}`}>{m.status_note}</p>
                          )}
                        </button>
                      );
                    })}
            ...
            {/* No machines yet — prompt to add */}
            {savedMachines.length === 0 && (tbEmail || localStorage.getItem("tb_email")) && (
              <div className="mb-1 rounded-lg border border-dashed border-zinc-700/50 px-3 py-2.5 text-center">
                <p className="text-[11px] text-zinc-600">No saved machines yet.</p>
                <a href="/toolbox" className="text-[11px] text-sky-400 hover:text-sky-300 underline underline-offset-2">Add machines in Toolbox →</a>
              </div>
            )}
            ...
            <div className="relative">
              <Input
                ...
                className="text-sm"
              />
              {machineDropOpen && (machineResults.length > 0 || machineQuery.length >= 1) && (
                <div className="absolute z-50 w-full mt-1 rounded-md border border-zinc-700 bg-zinc-900 shadow-xl max-h-60 overflow-y-auto">
                  {machineResults.length === 0 && machineQuery.length >= 1 && (
                    <div className="px-3 py-2 text-sm text-zinc-500 italic">No machines found for "{machineQuery}" — try brand + model (e.g. Haas VF-2)</div>
                  )}
   ...
   (no sub-spindle on this machine)`;
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg bg-zinc-800/40 border border-zinc-700/30 border-l-4 border-l-emerald-500 p-3 space-y-3">
              ...
              <div className="pt-2 border-t border-border/50 space-y-0">
                ...
                {(form.machine_type === "vmc" || form.machine_type === "hmc" || form.machine_type === "hbm" || form.machine_type === "5axis") && (
                  <div>
                    <div className="flex items-center gap-3 py-2 border-b border-border/30">
                      <Tooltip>
                        <TooltipTrigger asChild>
            ...
            <div className={`rounded-lg border p-3 ${pdfExtracted ? "border-amber-500 bg-amber-950/20" : "border-dashed border-gray-600"}`}>
              {pdfExtracted ? (
                <div className="space-y-2">
                  ...
                  {!(form.shank_dia > 0 && form.shank_dia > form.tool_dia * 1.05) && !(form.lbs > 0 && form.loc > 0 && form.lbs >= form.loc * 1.5) && <div className="flex items-center gap-2 pt-1">
          ...
          {form.mode === "deep_pocket" && (
            <div className="space-y-4 mt-2">
              ...
              {/* ── Tooling section: auto-generated by default; toggle to override with a specific tool ── */}
              <div className="rounded-lg border border-zinc-700/60 bg-zinc-900/40 px-3 py-2.5 space-y-2">
                <div className="flex items-center gap-2 pb-1.5 border-b border-zinc-700/40">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-300">Tooling</span>
                  {!dpSpecialTool && (
                    <span className="text-[11px] text-emerald-400">✓ Standard tooling will be auto-generated by the sequencer for your pocket geometry above.</span>
                  )}
                  {dpSpecialTool && (
                    <span className="text-[11px] text-orange-300">⚙ Special tool mode — sequencer skipped, your uploaded tool will be used.</span>
                  )}
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-semibold text-orange-400">Use specific tool (override sequencer)</span>
                    <span className="ml-2 text-[10px] text-zinc-500">— have a specific Core Cutter tool already in hand? Toggle on and upload its print to use it instead.</span>
                  </div>
                  <button type="button"
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-96 shadow-2xl" onClick={e => e.stopPropagation()}>
          ...
          {(!tbEmail && !localStorage.getItem("tb_email")) && (
            <div className="mt-3 text-center space-y-1">
              <p className="text-xs text-amber-400">You'll be asked to sign in to your Toolbox first.</p>
