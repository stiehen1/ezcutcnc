...
export default function Mentor() {
  ...
  function getDynamicPresets(mode: string, iso: string, flutes: number, dia: number, loc: number): {
    ...
    const flatDoc: Record<string, { low: number; med: number; high: number }> = {
      finish:     { low: 0.1, med: 0.5, high: loc > 0 && dia > 0 ? loc / dia : 2.0 },
...
<div class="header">
  ...
  <div class="header-center">
    <strong>Produced with EZCutCNC by Core Cutter LLC</strong>
    ${now}<br>
...
<div class="disclaimer">
  ...
  <br><strong>© ${new Date().getFullYear()} Core Cutter LLC. All Rights Reserved.</strong> EZCutCNC is a proprietary tool of Core Cutter LLC. Unauthorized reproduction or distribution is prohibited.
</div>
<script>window.onload = function() { window.print(); };<\/script>
  ...
  function buildResultsText(): string | null {
    ...
    const lines: string[] = [];
    lines.push("Produced with EZCutCNC by Core Cutter LLC");
    const _now = new Date();
    ...
    lines.push("");
    lines.push("════════════════════════════════════════");
    lines.push(`© ${new Date().getFullYear()} Core Cutter LLC. All Rights Reserved.  |  EZCutCNC  |  corecuttertool.com`);
    return lines.join("\n");
  }
