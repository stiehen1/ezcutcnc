...
const MILLING_MODE_TIPS: Record<string, Array<{ title: string; body: string }>> = {
  hem: [
    ...
    { title: "Entry strategy protects tool life.", body: "Never drop straight into full-width material. The best HEM entry is a sweep-in (tangential arc lead-in) — the tool rolls into the cut from the side, starting at zero chip thickness and building engagement gradually. This is the gentlest possible entry and the one the app recommends first. Where sweep-in isn't possible, use a helical ramp or 2–5° ramp-in; pre-drilled entry for deep work. CAM defaults often spike engagement at inside corners and chip tools silently — add corner smoothing and eliminate 90° inside corners from your toolpath." },
    ...
    { title: "When can you run dry?", body: "Dry machining works when heat stays in the chip and the coating handles it. Best candidates: cast iron (dry preferred — see above), aluminum with DLC coating (D-Max) using air blast only — DLC prevents built-up edge without lubrication, and high-SFM steel cuts with AlTiN/TiAlN coatings where the coating itself is the heat barrier — flood coolant can actually cause thermal shock here. You cannot run dry in: HRSA (Inconel, titanium, cobalt alloys), stainless steel, any deep pocket or blind hole where chips can't escape, or any interrupted cut where the tool needs lubrication to prevent galling at re-entry. If chips are coming out discolored blue or black — you're dry when you shouldn't be." },
  ],
  traditional: [
