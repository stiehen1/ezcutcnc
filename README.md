...
- **Personalized sender.** When a logged-in user emails results, the message now shows as **"[User] via Core Cutter"** with **Reply-To** set to that user, so it reads as coming from them. The actual From address stays the authenticated sender (display-name only), so deliverability is unchanged. (Putting the user's own address in From would fail SPF/DKIM/DMARC — never done.) Fixed a prior Reply-To bug that pointed at the recipient instead of the sender; display name is sanitized against header injection.
