...
### Drill SFM recalibration + three drilling-speed bug fixes
- **Austenitic stainless drilled too slow.** The delivered drill SFM for 304 (base × coolant-fed bonus) landed only ~107 at Ø.236 against a shop-reference through-coolant band of 150–220 for austenitic. The base was an external-coolant anchor set too low. Raised the 304 base 80→95 (delivered ~130 @ Ø.25 / ~171 @ Ø.50) — a safe **starting** value that never overshoots at large diameter, where the coolant bonus climbs toward 2×.
- **Bug: hardened steel got a coolant speed bonus it was meant to be denied.** The coolant-fed class table *had* `heat_limited` entries for `hardened_lt55`/`hardened_gt55`, but they were keyed by material-name while the lookup is by **material group** (`Steel`) — so the entries were dead code and hardened steel wrongly received the full chip-limited bonus (delivered inflated to ~135 SFM). Moved them into the material-keyed override table; hardened steel is now correctly flat 1.0× (coolant preserves tool life, doesn't add speed).
- **Bug: `_NO_HRC_PENALTY` divergence in the drill path (same class of bug as the June feedmill/reaming fix).** The drill path kept its own, narrower hardness-exclusion list that was missing `cpm_10v`, `stainless_440c`, `stainless_15_5/ph/13_8`, `duplex/superduplex`, and `armor_*`. Those materials got a hardness derate on top of an already-hardened base — a double-count that produced nonsense speeds (CPM-10V delivered **12 SFM**, 440C **25 SFM**). Routed the drill path through the central `_NO_HRC_PENALTY` frozenset so it can't drift again; CPM-10V now delivers ~61, 440C ~95.
- **Base SFM raised toward shop-reference low-end** for materials that were running conservative after the bug fixes: annealed tool steels (A2/H13 →100, D2 →85, steel_tool →98), 316 (→90), 17-4 PH (→80), Ti-6Al-4V (→55), and the bronzes (manganese/silicon →180). All targeted at the *low* end of the reference band — these are starting speeds; users push up via the SFM presets as chips and finish confirm.
