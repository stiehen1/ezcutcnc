...
### Steel material picker: the condition is now part of the choice, and M2 stopped reading as alloy steel
An uploaded print calling out **M2 Steel** came back as Alloy Steel at 32 HRC — 343 SFM against a high-speed-steel punch. The alias map had M2 mapped correctly, but the PDF extraction prompt is a *separate* list and never named M2 or HSS at all, so the model matched the word "STEEL" and fell through rule (d) to `steel_alloy`. That is the documented prompt-drift failure mode, and it is why the prompt now names every HSS grade explicitly and warns that an "Mx"/"Tx" callout is never a plain alloy steel.

Auditing the rest of the steel list turned up two harder problems. **A2 defaulted to 36 HRC while its own valid range was 54–62; S7 defaulted to 38 against 54–60** — both pickers opened on a value the app itself rejected. And the hardness field was **inert for four of six tool steels**: A2, S7, H13 and CPM 10V sat in `_NO_HRC_PENALTY` with a flat `BASE_SFM`, so an annealed blank and a 62 HRC part quoted identical speeds. Only D2 responded, because D2 alone had a real HRC curve (`d2_sfm_absolute`). That matters because these grades are almost always **machined annealed and hardened afterwards** — the condition the picker advertised was the one you *don't* cut in.

`_anneal_hard_sfm()` generalizes the D2 curve so each grade supplies its own annealed and hardened anchors, and `apply_sfm_hardness()` now scales every per-operation table (drill, ream, keyseat, dovetail, feed-mill) by that curve's ratio. The drill path had its own inline D2-only special case and was bypassing the central function entirely — a 62 HRC M2 punch was being drilled at annealed speed. Twelve grades now carry curves; ten new keys were added: **O1, W1, HSS (M2/M3/M7/T1), HSS-Co (M4/M42/T15), CPM 3V, CPM 15V, bearing steel (52100), maraging (18Ni), high-carbon (1060–1095), and carburizing pre-case (8620/9310/4320)**. The carburizing grades came *out* of the 32 HRC alloy-steel line: pre-case they're low-carbon and gummy, and post-case the cutter sees a hardened case — neither is 4140.

For annealed M2 the three answers now differ sharply: `steel_alloy` gave 343 SFM / 58 IPM (2.3× too fast), `hardened_gt55` gave 240 / 10.4 (right speed, half the feed, because hard-milling IPT starves an annealed blank), and the new key gives **150 SFM / 19.4 IPM**.

ISO P went from 10 options to 20, so the dropdown is now grouped with `<optgroup>` — **Carbon & Alloy Steel / Tool Steel / High-Speed Steel / PM Tool Steel** — via a `group` field on each subcategory. ISO M (14) and S (11) and H (6) got the same treatment: Austenitic / Ferritic / Martensitic / PH / Duplex / Work-Hardening, Titanium / Ni / Fe-Co / Ni-Co, and Hardened Steel / Armor Plate. Short lists (N1, N2, K, O) render flat as before. Grouped entries must stay contiguous or `<optgroup>` emits the same heading twice — 316 was sitting below the martensitic block and had to move up next to 304.

Also fixed while in here: `ISO_MAP` in the peer-tool query declared the named tool steels **twice**, once as "P" and again as "H". The later duplicate silently won, so every tool-steel job was getting ISO H peer filtering and preset lookups instead of P.

**Calibration status:** the annealed anchors are the previously calibrated `BASE_SFM` values, so default-condition output is unchanged. The *hardened* anchors, and all constants for the ten new keys, are ESTIMATES — extrapolated from the D2 curve's ratio and each grade's published machinability rating, and marked as such in the code. They need shop runs to become real data.
...
The shop anchor these numbers now reproduce, from tier-1 aerospace production on a 1.000" 3-flute solid carbide: **26,000 rpm** — the spindle's *peak-power* band, deliberately below its 33k max, because dropping back buys torque — which is 6,807 SFM on a 1" cutter. 600 IPM at 0.0077 ipt gives 300 in³/min; 800 IPM at 0.0102 gives 400. Both at 1.000" DOC (1×D) and **0.500" WOC (50% of Ø)** — a heavy radial bite at a heavy chip load, where the chip is the heat sink. That is not light-radial HEM, and treating it as such was the second half of the problem: the stock `HEM_IPT_MULT` of 2.0 presumes the 1.7–2.1× chip thinning of a 6–10% radial arc, but at 50% WOC there is essentially none, so it stacked on an already-full chip and produced ~800 in³/min at 223 hp — double the real recipe.

`hs_spindle_sfm_ceiling()` raises the ceiling to the anchor, gated on **all** of: a Makino MAG (brand + model regex, so a "MAGNUM" can't trip it), an aluminum-family material, ≥18,000 rpm, ≥40 hp, and a shrink/hydraulic/Capto holder — a side-lock at 26,000 rpm is a broken tool, not a fast one. Any gate failing returns `None` and the conventional band applies untouched, so no VMC output moved. `max()` on the ceiling means the gate can only ever *open* the band. The preset ladder re-anchors so `max_mrr` lands on the ceiling, while `balanced` and slower deliberately keep the conventional value — reaching for peak power should be a deliberate choice, not the default recommendation.

The WOC ladder on a MAG becomes **25 / 35 / 45%** in place of the light-radial 12/15/18, with 45 sitting just under the validated 50 for the same reason the titanium hybrid offers 25 when 27 was run: the button stays a touch conservative and the operator can nudge to the proven value. The engine's own default is the 35% med step so the button row and the computed feed can't drift. This applies to **traditional roughing as well as HEM** — at 50% radial engagement there's no trochoidal arc left, so the two strategies describe the same physical cut, and gating it to HEM would have handed the same machine 398 in³/min under one radio button and ~30 under the other. DOC is deliberately *not* opened to full LOC: the anchor ran 1×D, comfortably inside the existing 1.5×D three-flute cap, and MAG tooling is mostly reduced-neck, so stacking maximum axial against a heavy radial bite would load the thinnest section of the tool.

Only the 26,000-rpm point is shop-validated. The gating thresholds and the ramp between spindle classes are estimates extrapolated from it, marked as such in the code alongside the un-calibrated fixture constants. `machine_brand` / `machine_model` are new schema fields, populated on catalog machine select and blank for a manual setup — absent or unrecognized values just fall through to the conventional numbers. MAG select also now forces zero-point / pallet workholding, matching the existing shrink-fit force on that platform.

Separately: on a reduced-neck tool the "Minimum stickout = X (LBS — shank to neck)" stability step fired unconditionally, because its gate (`_lbs > 0 and _min_so > 0`) never compared against the *actual* stickout. Setting the stickout to the minimum by hand left the step sitting there telling you to shorten a value that physically can't shorten. It now also requires the current stickout to be above the floor, with 0.0005" of slack so a stored 3.2501 against a 3.250 floor still reads as at-the-floor. Standard non-necked tools and the estimated-print path are unchanged.

### Tombstone is now a 5-axis workholding option, and the zero-point button lost its brand name
All eight Makino MAG models (MAG A6, A8, A8.H, MAG1, 3, 3.EX, 3.W, 4) are typed `machine_type = '5axis'`, but `tombstone` was only offered on `hmc`, `gantry`, `hbm`, and `double_column`. A shop running a tombstone on a MAG — the normal setup for high-speed aerospace aluminum on that platform — had no button for it and had to settle for Rigid Fixture. Worse, the machine-select handler actively coerces the selection to a machine-type default when the current value isn't in the allowed list, so switching a saved setup to a 5-axis machine silently dropped a tombstone choice.

`tombstone` is now in the `5axis` list in both `_allowedWH` (the coercion map) and `WORKHOLDING_ALLOWED` (the Setup Score normalization set) — these two must stay in sync or the sub-score normalizes against the wrong option pool. The button sits between Rigid Fixture (0.80) and Pyramid (0.84) to hold the stiffest→softest ordering that `WORKHOLDING_RIGIDITY_VALUE` expects. No engine change was needed: `tombstone: 0.82` already existed in `WORKHOLDING_COMPLIANCE`, so the physics and scoring paths were wired all along and this was purely a UI gating problem. The tooltip read "mounted to HMC pallet," which would have been wrong on a 5-axis machine, and now names both.

Separately, the zero-point button read **Zero-Point / RockLock**. RockLock is one vendor's brand (Jergens) while every other button in the row is generic, so it read as an endorsement to shops running EROWA or VERO-S. It's now **Zero-Point / Pallet System**, with the vendor names kept in the tooltip where they inform rather than endorse. The `zero_point` enum key is unchanged — renaming it would orphan every saved setup in `toolbox_sessions` that stores the string. The 5-axis rigidity-chain explainer carried the old label *and* was still missing Tombstone; it now matches the button order.
...
- **Workholding** lists cap at 6 rows, keeping the nearest upgrades plus the stiffest option so both the cheap next step and the ceiling are visible; the remainder is counted, not silently dropped (soft jaws on a lathe has 12 stiffer options). The **holder** ladder is never capped — it tops out at 7 rows and every rung is a distinct interface a shop might own. Capping it dropped shrink fit while keeping Capto, which is both the wrong rung to lose and confusing beside the "capital equipment" tag on the row that survived.
...
It also printed two answers to one question. The starred table read "Stability 64% → 52% ✓" while the on-screen panel said the 5-flute lands at 61.1% of limit — different engines, same label. And when the scorer picked a coating variant of the tool already in the spindle, the sheet was telling a customer to buy a new EDP for the tool in their hand.

The **stability advisor now headlines the block** — it's the app's single on-screen source of recommendations, so the sheet matches the screen:

- The advisor's first tool pick gets the star, its fix label and detail, and the Current-vs-Recommended table; remaining picks list under the fix each belongs to, and `+N more variants` carries over.
- Table numbers come from the **same estimator as the hover preview** (`estStepToolPreview`, hoisted to module scope so the PDF and the results panel can't drift), labeled as a geometry estimate with a pointer to re-run for exact figures.
- The scorer is **demoted to "Alternate — best overall catalog match"**, and suppressed when it duplicates the headline or the tool already being run. Genuinely useful — it can change diameter and flute count — but as an alternate, not the recommendation.
- The toggle appears when **either** source has a pick, not just the scorer, so setups whose only tool advice comes from the advisor can still print the block.
...
The Machining Tips accordion sat between the "Recommendation" heading and the first number — background reading on the strategy, standing between the user and their parameters. Its place goes to a **Tool Being Run** reference strip. The numbers below are only trustworthy if they're for the *right* tool, so the results restate what the engine actually ran against before showing any of them.
- **Specs ordered the way they read off a print**, front of the tool back: Cut Ø · Shank Ø · LOC · **LBS** · **Neck Ø** · OAL · Flutes · Corner Condition · Coating · Tool Design · Center Cutting. LBS (Length Below Shoulder) is the defining dimension of a necked tool — it's both the reach and the stickout floor — and it wasn't shown at all; the reduced neck Ø follows immediately after it since it's the weak link in the LOC→LBS span.
- **Chamfer mills were excluded from the card entirely.** They now get their own spec set instead of endmill fields that read blank or wrong: included angle **and** per-side (prints call it out both ways, and confusing them doubles the chamfer), tip Ø or "Sharp / pointed", plus a computed **usable edge length** — the number to check against the requested chamfer width — using the same `radialReach / sin(halfAngle)` as the existing chamfer-upgrade suggestion.
- **Uploaded customer prints** identify as `CC# <number> | Special` with taper angle/length on tapered specials, and an amber note when stickout was estimated from the drawing rather than catalog geometry — placed next to the specs it qualifies, not only in the stability advisor.
- **`EDP# 505221 | Series VST5` in bold yellow** — it's the number you order against, and it was rendering as a grey caption.
- **Coating shows "Uncoated" rather than omitting the row** when blank; a missing coating line reads as "we didn't know" instead of "there is none". **Tool Design** (Variable Pitch / Helix / both / Standard) was only inferable from the series name despite the engine scoring it at up to 1.75× the stability limit.
- **Description cleanup.** Catalog strings tail off with the corner condition and "Reduced Neck", both of which the fields below already state in numbers. Stripping the latter with `/reduced neck.*$/` cut from *inside* `"(Reduced Neck)"` and left a dangling `(` — now removes the parenthesised form as a unit, with a backstop for any unclosed bracket, plus a word-boundary cap at 72 chars so CSS truncation can't sever a word. Verified against seven description shapes; legitimate parentheticals like `(Var Helix)` survive.
- **Sized down to a reference strip** — small type, tight padding, muted border. At full size it competed with the cutting parameters it sits above. Stickout is deliberately excluded as a setup value the stability panel already reports with L/D.
...
- **Feed needed its own multiplier, and this was the substantive fix.** `HEM_IPT_MULT = 2.0` is premised on heavy chip thinning — at 7% WOC the chip thins 1.96×, so doubling programmed feed merely restores a sane chip. At 25% WOC thinning is only 1.15×, so the same 2.0 stacked on an already-thick chip and asked for **.0043 fpt / 45 IPM** against the .0027 / 27 that actually ran. Hybrid uses **1.35**, confirmed from two independent directions: solving for the reported adj FPT needs 1.34–1.37 across 25–27% WOC, and the separately-stated 0.5%×D base FPT × chip thinning lands .0028.
- **Own SFM base (227) and own preset spread.** Reusing the shared Titanium bias row would have dragged classic HEM to 414 SFM, so hybrid gets `_SPEED_PRESET_HYBRID_HEM`: **180 / 204 / 227 / 245 / 261**. The flag threads through `speed_preset_factor` / `biased_sfm` / `speed_envelope` so the manual-override clamp band matches the buttons.
- **HRSA radial cap lifted 15% → 30% for hybrid only**; otherwise that cap silently clamped the recipe back down. DOC caps at **2×D** (not 3×D) — heavy radial trades axial.
- **Titanium-only, not ISO S.** Inconel/HRSA share the category but are separately shop-calibrated at 2–3% WOC; heavy radial there has no data behind it.
- **Ti speed ladder retuned alongside**: upside pulled in (max_mrr 1.18 → 1.055, so classic tops at ~380 rather than 425), and the rubbing floor raised **115 → 180 SFM** — 115 let Max Life land at 125 SFM off the hybrid base, deep in the zone where the edge plows instead of cutting and life *shortens*.

### Low shank grip now warns about runout, and forces a reduced-neck recommendation
`grip_state()` already modelled the rigidity loss when a tool is pulled out so far there's little shank left to hold (quadratic falloff below the 1.2× shank-Ø minimum, down to a 0.22 floor). Two things were missing.
- **Runout, stated in words.** Below the minimum the shank can *tip* in the collet, not just flex, so tip TIR grows with the lever arm — which the rigidity multiplier doesn't capture. Rather than invent a numeric TIR adder with no calibration behind it (and risk double-penalizing a setup the model already derates), the "red" grip warnings now name the failure mode: more runout, uneven flute loading, corner chipping and short life *before* it ever shows as deflection.
- **Reduced-neck was gated purely on deflection**, so the case that motivates it most stayed silent — a stiff tool taking a light cut can sit at 23% of the flex limit while holding on 0.9× shank Ø. It now also fires when grip is red **and** pushing the tool in to its own floor still can't restore the minimum. That last clause is what keeps it honest: when pushing in *does* fix it, the free fix stands alone and no tool purchase is suggested. The grip path bypasses the 15%-better-flex test, which is the right bar for a deflection-driven swap and the wrong one when the win is holder engagement.

### How to Use — TSC explained, editable machine fields, rigidity-ordered pickers
- **"TSC" is now spelled out.** Users were asking what it meant. The coolant sections lead with the expansion (Through-Spindle Coolant — pumped through the spindle and out of the tool at the cutting edge; TSC300/TSC1000 ≈ 300/1000 psi), say to select it if you have it, and quote the tool-life credits the engine actually applies: **flood 1.25× / TSC300 1.35× / TSC1000 1.50×** vs dry. Deliberately does **not** claim TSC raises milling speeds — verified that in milling it moves tool life only, while in **drilling** it lifts SFM ~20% at 300 psi and ~30% at 1000 psi (Ti 0.375": 70 → 84 → 91), which is stated as the drilling case.
- **Machine fields are editable, and the guide now says so.** A user whose CNC isn't among the 800 in the database had no way to know the app still works fine for them — they only need Max RPM, Machine HP, Spindle Drive, Machine Type and Spindle Interface to match. Also covers the case where a listed machine's specs don't match how theirs is actually configured (retrofits, spindle swaps).
- **Max RPM Use reframed as a preference.** The old copy gave only the vibration rationale; the common reason is simply that plenty of shops don't like running a spindle wide open. Now the operator's call with 95% as a default, keeping older-spindle / high-runout / long-reach as the technical case for going lower.
- **Tool Holder and Workholding lists are noted as ordered most rigid / most precise first.** Verified both ways before claiming it: holders render Capto → ER matching `TOOLHOLDER_RIGIDITY` 1.20 → 1.00, and the workholding arrays were already commented "stiffest→softest to match `WORKHOLDING_RIGIDITY_VALUE`" (rigid_fixture 0.80 compliance through soft_jaws 1.20).
...
- **Field renamed `Part Diameter at Overhang` → `Smallest Section in the Overhang — Dia. or Thickness`.** The old title taught the wrong thing twice over: "at Overhang" reads as "measure at the overhang" (the exact pick the note talks you out of), and "Diameter" doesn't describe the square bar, rectangular blocks, and plate that hang out of fixtures all day. The new label carries the rule ("Smallest") and admits non-round sections, while keeping "Overhang" as the shared anchor with the length field above it.
- **Non-round parts now have real guidance, and it's the counterintuitive half.** On a square or rectangular section it's the **side facing the cutting force**, not the bigger one — a part only resists in the direction it's being bent. Added a dedicated square/rectangular bullet (a 2"×1" block pushed across its narrow face is a `1.000`), and the plate bullet now quantifies the trap: on a 4"×0.5" web, entering `0.500` lands near the true bending stiffness while entering `4.000` overstates it by **~300×**. Verified against `I = b·h³/12` for the real section vs the model's `π·d⁴/64`.
- **Entering non-round parts this way is provably conservative**, which the note now says outright: a square bar reads ~59% of true stiffness and a thin web lower, so you get a slightly cautious feed rather than an unsafe one. The `workpiece_deflection()` docstring in `engine/physics.py` was updated to match — it previously said only "conservative; a tube is stiffer, a thin web is softer" without stating what to actually enter.
- **Diameter tooltip rewritten** to lead with the error magnitude (a stepped part entered at its chucked-up diameter can read 16× stiffer than it is) and the three common cases, instead of the old solid-round-only "smallest diameter along the overhang".
